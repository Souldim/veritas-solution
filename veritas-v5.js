...
function renderProjects(){
  ...
  grid.innerHTML=SITE_CONTENT.projects.map((p)=>`<article class="project" data-image="${p.image}"><img src="${p.image}" alt="${p.title[currentLang]}"><div class="project-info"><strong>${p.title[currentLang]}</strong><span>${p.city || p.category || ''}</span></div></article>`).join('');
